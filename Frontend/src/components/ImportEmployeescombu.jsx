...
const ImportEmployeeshc = () => {
  ...
  const fetchStatistics = async (selectedMonth = enteredMonth) => {
    ...
    try {
      const employeesResponse = await axios.get(`http://localhost:3500/employeescombu/month/${monthToFetch}`);
  ...
  const handleDeleteStat = async (id) => {
    try {
      await axios.delete(`http://localhost:3500/statisticscombu/${id}`);
  ...
  const handleDelete = async (e) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:3500/employeescombu/month/${deleteMonth}`);
