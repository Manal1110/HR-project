const Employeehc = require('../models/employeeshc');
const multer = require('multer');
const xlsx = require('xlsx'); // For handling Excel files
const path = require('path');
const fs = require('fs');

const createEmployeehc = async (req, res) => {
  try {
    const newEmployee = new Employeehc(req.body);
...
// Get all employees
const getEmployeeshc = async (req, res) => {
  try {
    const employees = await Employeehc.find();
...
const getEmployeehcById = async (req, res) => {
  try {
    const employee = await Employeehc.findById(req.params.id);
...
// Update an employee
const updateEmployeehc = async (req, res) => {
  try {
    const employee = await Employeehc.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!employee) return res.status(404).json({ message: 'Employee not found' });
    res.status(200).json(employee);
  } catch (error) {
    res.status(400).json({ message: error.message });
...
// Delete an employee
const deleteEmployeehc = async (req, res) => {
  try {
    const employee = await Employeehc.findByIdAndDelete(req.params.id);
    if (!employee) return res.status(404).json({ message: 'Employee not found' });
...
const importEmployeeshc = async (req, res) => {
  try {
    upload.single('file')(req, res, async (err) => {
      ...
      for (const item of data) {
        try {
          const newEmployee = new Employeehc({ ...item, month });
...
const getEmployeeshcByMonth = async (req, res) => {
  try {
    ...
    const employees = await Employeehc.find({ month });
